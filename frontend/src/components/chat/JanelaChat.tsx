    ...
    return(
        <div className="flex flex-col bg-zinc-300 rounded-2xl text-black overflow-hidden">
            ...
            {mensagens.length === 0 ? (
                ...
                <div className="flex flex-col p-2 gap-2 min-h-[400px] sm:min-h-[500px] sm:max-h-[400px] overflow-y-scroll">
                    {mensagens.map((mensagem, i) => {
                        const mesmoAutor = i > 0 && mensagens[i - 1].autor === mensagem.autor
                        return (
                            <BalaoMensagem key={BalaoMensagem.id} mensagem={mensagem} omitirAutor={mesmoAutor}/>
                        )
                    })}
                    {pensando && ( 
